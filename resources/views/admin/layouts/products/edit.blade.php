	...
	<div class="col-md-12 m-auto">
		...
		<div class="col-lg-10 col-md-12 col-sm-12 row">
			...
			<div class="form-group">
				...
				{{ Form::select('brand_id', $brands, $product->brand_id,['class' => 'form-control','placeholder'=>"Chose Brand"]) }}

				<span class="text-danger">{{ $errors->first('brand_id')}}</span>
			</div>
			<div class="form-group">
				{{ Form::label('Category: ','',['class' => 'font-weight-bold']) }}
				{{ Form::select('category_id', $categories, $product->category_id,['class' => 'form-control','placeholder'=>"Chose Category"]) }}

				<span class="text-danger">{{ $errors->first('category_id')}}</span>
			</div>
			<div class="form-group text-right">
				<a class="btn btn-info mt-3" href="{{ route('product.index') }}" title="back"><i class="fas fa-arrow-left"></i> View List</a>
